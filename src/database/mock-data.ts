...
import { Club, ClubConstructorData } from './club'
import { Course, CourseConstructorData, Duration } from './courses'
import { fireDB, FrostApp } from './frost'
import { Professor, ProfessorConnectOptions } from './professor'
import { Student, StudentConnectOptions } from './student'

/*
 * A set of clubs at a certain school
 */
export const clubsData: ClubConstructorData[] = [
    {
        id: 'club-glee-01',
        name: 'Glee',
        type: 'CREATIVE',
    ...
    { id: 'club-chess-01', name: 'Chess', type: 'STEM', roomId: 'rec-room-06' },
    {
        id: 'club-drama-01',
        name: 'Drama Club',
        type: 'CREATIVE',
 ...
 */
export const coursesData: CourseConstructorData[] = [
    ...
    },
    {
        ...
        department: 'basic-science/math',
        difficultyLevel: 'ADVANCED_PLACEMENT',
    },
    ...
    for (const data of professorsBasicData) {
        ...
        yield {
            professor: new Professor({
                id: `${fName}-${mName}-${lName}-${Math.floor(Math.random() * 100000)}`,
...
export function* studentsGen(count:number = 1) {
    ...
    for (let index = 0; index < count; index++) {
        ...
        if (age <= 15) year = 'FRESHMAN'
        else if (age === 16) year = 'SOPHOMORE'
        else if (age === 17) year = 'JUNIOR'
        else year = 'SENIOR'
        const fName = faker.name.firstName()
        const lName = faker.name.lastName()
...
export const addClubs = async (preform = true) => {
    ...
    for (const clubData of clubsData) {
        ...
        const { map } = await FrostApp.ClubApi.getAddMap(club)
        clubs = { ...clubs, ...map }
    }
...
export const addCourses = async (preform = true) => {
    ...
    for (const courseData of coursesData) {
        ...
        const { map } = await FrostApp.CourseApi.getAddMap(course)
        courses = { ...courses, ...map }
    }
...
export const addStudents = async (preform = true) => {
    ...
    for (const {student,connect} of studentsGen(10)) {
        ...
        const { map } = await FrostApp.StudentApi.getAddMap(student, connect)
...
export const addProfessors = async (preform = true) => {
    ...
    for (const profData of professorsGen()) {
        const { map } = await FrostApp.ProfessorApi.getAddMap(
            profData.professor,
            profData.connect,
        )
